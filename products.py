...
def read_file(filename): #將內容物都補一個tab(或是四個空白鍵)
    ...
    with open("products.csv", "r", encoding = "utf-8") as f :
        for line in f:
            if "商品,價格" in line:
                continue #繼續，跳過這一次的循環，從下一輪開始跑
            name,price = line.strip().split(",") # 由左而右執行，先把換行符號去掉(strip)，再切割(split)，以逗點為依據切割
...
def user_input(products):
    ...
    print(products) 
    return products #因為我們想要把使用者新增的品項也裝入 products ，所以這個 def 也需要 return
...
def write_file(filename, products): # 因為需要一筆一筆讀取 ，所以需要設二個參數
...
def main(): # 把主要執行的程式碼收納成一個主程式(main function)，當作切入點
    ...
    if os.path.isfile(filename): # 檢查檔案在不在，因為只使用一次且不會重複執行，故不額外定義一個function
        ...
        print("找不到檔案")
    products = user_input(products) # 把 read_file 得到的結果儲存在 products 這個變數 ，再把它丟到 user_input() 執行 ，得到的結果再放入左邊的 products
